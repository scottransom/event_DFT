...
/* Prepare the static variables and vectors for efficient  */
/* calculation of DFT points based on events.  The FT will */
/* begin at frequency  'lof' (Hz) and continue upwards in  */
/* frquency by stepsize 'df'.  There are 'numevents' input */
/* events with times 'events' (s).  Up to 'maxharmsum'     */
/* harmonics will be summed.                               */
{
  int ii, jj;
...
/* Calculate the next frequency of the event DFT. */
/* A pointer to the vector of normalized Fourier  */
/* amplitudes (of length maxharmsum) is returned. */
/* '*freq' contains the frequency (Hz) that it    */
/* corresponds to (dundamental).  prep_eventdft() */
/* must have been called to initialize the calcs. */
{
  int ii, jj;
...
/* Return a set of normalized Fourier amplitudes   */
/* calculated using an event DFT for a set of 'nn' */
/* events 'tt'.  The returned array is allocated.  */
{
  int ii;
  double freq;
  fcomplex *famp, *amps;
...
float *periodogram(double *xx, double *tt, int nn, 
		   double lof, double df, int numf)
/* Return the normalized Lomb-Scargle Periodogram powers */
/* of 'numf' frequencies (Hz) from the lowest freq 'lof' */
/* upwards by stepsize 'df'.  There are 'nn' input data  */
/* points with amplitudes 'xx' and times 'tt' (s).  The  */
/* returned power vector is dynamically allocated.       */
{
  int ii, jj;
  float *pows;
